<template>
	<uni-nav-bar :fixed="true"  status-bar left-icon="left" title="作业练习" @clickLeft="home()" background-color="#F5F5F5" :border="false" />
	...
	<view class="sentence_box" >
		<view v-for="item in word_list" class="sentence">
		  ...
		  <text v-if="item.type=='select'" >{{ "______" }}</text>
		</view>
	</view>
	
	<text class="translate_text" v-if="options.translate_show">{{essay.translate}}</text>
	
	<view class="select_grid">
		<view style="margin-top: 5px;margin-left: 5px;" v-for="item in essay.nine_select" :key="item.id" >
		  <button v-if="item.status==='error'" class="error_btn" @click="select_btn(item)">{{ item.text }}</button>
		  <button v-else class="nine_btn" @click="select_btn(item)">{{ item.text }}</button>
		</view>
	</view>
...
onLoad(e=>{
	result_sentence.homework_id = e.homework_id
	result_essay.homework_id = e.homework_id
